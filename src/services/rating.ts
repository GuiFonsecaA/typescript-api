...
export class Rating {
  ...
  public getRatingForSwellSize(heigth: number): number {
    if (
      heigth >= waveHeigths.ankleToKnee.min &&
      heigth < waveHeigths.ankleToKnee.max
    ) {
      return 2;
    }
    if (
      heigth >= waveHeigths.weistHigh.min &&
      heigth < waveHeigths.weistHigh.max
    ) {
      return 3;
    }
    if (heigth >= waveHeigths.headHigh.min) {
      return 5;
    }
    return 1;
  }
